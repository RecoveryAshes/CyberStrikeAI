...
【参数】tasks（字符串数组）或 tasks_text（多行，每行一条）二选一；每项是一条将来由系统按队列顺序执行的指令文案。agent_mode：eino_single（Eino ADK 单代理，默认）、agent_runtime（独立 Rust Runtime，需启用 agent_runtime）、deep / plan_execute / supervisor（需系统启用多代理）。非”把主对话拆给子代理”。schedule_mode：manual（默认）或 cron；cron 须填 cron_expr（5 段，如 “0 */6 * * *”）。
...
【执行】默认创建后为 pending，不自动跑。execute_now=true 可创建后立即跑；否则之后调用 batch_task_start。Cron 自动下一轮需 schedule_enabled 为 true（可用 batch_task_schedule_enabled）。`,
		...
		InputSchema: map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"agent_mode": map[string]interface{}{
					"type":        "string",
					"description": "执行模式：eino_single（Eino ADK，默认）、agent_runtime（独立 Rust Runtime，需启用 agent_runtime）、deep/plan_execute/supervisor（Eino 编排，需启用多代理）",
		...
		agentMode := config.NormalizeBatchAgentMode(mcpArgString(args, "agent_mode"))
	...
	reg(mcp.Tool{
		...
		InputSchema: map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"agent_mode": map[string]interface{}{
					"type":        "string",
					"description": "代理模式：eino_single、agent_runtime、deep、plan_execute、supervisor",
					"enum":        []string{"eino_single", "agent_runtime", "deep", "plan_execute", "supervisor"},
				},
			},
			"required": []string{"queue_id"},
